...
namespace T1
{
    ...
    void Renderer::Init(GLADloadproc loadProcFunc)
    {
        ...
        T1::Texture *hdrMap = T1::Resources::LoadHDR("sky env", "textures/backgrounds/alley.hdr");
        T1::PBRCapture *envBridge = m_PBR->ProcessEquirectangular(hdrMap);
        ...
        */
        ...
        if (Shadows)
        {
            ...
            for (int i = 0; i < m_DirectionalLights.size(); ++i)
            {
                ...
                if (light->CastShadows)
                {
                    ...
                    math::vec3 dir = -light->Direction * 10.0f;

                    math::mat4 lightView = math::lookAt(dir, math::vec3(0.0), math::vec3::UP);
        ...
        for (auto it = m_PointLights.begin(); it != m_PointLights.end(); ++it)
        {
            if ((*it)->RenderMesh)
            {
                ...
                math::vec3 scalar = math::vec3(0.25f);

                math::scale(model, scalar);
        ...
        if (LightVolumes)
        {
            ...
            for (auto it = m_PointLights.begin(); it != m_PointLights.end(); ++it)
            {
                ...
                math::vec3 scalar = math::vec3((*it)->Radius);
        ...
        if (IrradianceGI && irradianceProbes.size() > 0)
        {
            ...
            for (int i = 0; i < irradianceProbes.size(); ++i)
            {
                ...
                if (m_Camera->Frustum.Intersect(probe->Position, probe->Radius))
                {
                    ...
                    math::vec3 scalar = math::vec3(probe->Radius);
    ...
    void Renderer::renderDeferredPointLight(PointLight* light)
    {
        ...
        math::vec3 scalar = math::vec3(light->Radius);
