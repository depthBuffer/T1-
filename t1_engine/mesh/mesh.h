...
namespace T1
{
    ...
    */
    class Mesh
    {
        // public for now for testing and easy access; will eventually be private and only visible to renderer (as a friend class)
    public:
    ...
    public:
        ...
        // not sure if these are required if we can directly set vertex data from public fields; construct several use-cases to test.
        ...
        void SetTangents(std::vector<math::vec3> tangents, std::vector<math::vec3> bitangents); // you can only set both tangents and bitangents at the same time to prevent mismatches
        ...
        void FromSDF(std::function<float(math::vec3)>& sdf, float maxDistance, std::uint16_t gridResolution);
