...
#include <t1_utility/string_id.h>
...
namespace T1
{
    ...
    Material* MaterialLibrary::CreateMaterial(std::string base)
    {
        ...
        if (found != m_DefaultMaterials.end())
        {
            ...
            m_Materials.push_back(mat); // a bit ugly for now, come up with a bettermemory management scheme for materials
            return mat;
        }
        else
        {
    ...
    void MaterialLibrary::generateDefaultMaterials()
    {
        ...
        glassMat->SetTexture("TexAlbedo", T1::Resources::LoadTexture("glass albedo", "textures/glass.png", GL_TEXTURE_2D, GL_RGBA), 0);
        glassMat->SetTexture("TexNormal", T1::Resources::LoadTexture("glass normal", "textures/pbr/plastic/normal.png"), 1);
        glassMat->SetTexture("TexMetallic", T1::Resources::LoadTexture("glass metallic", "textures/pbr/plastic/metallic.png"), 2);
        glassMat->SetTexture("TexRoughness", T1::Resources::LoadTexture("glass roughness", "textures/pbr/plastic/roughness.png"), 3);
        glassMat->SetTexture("TexAO", T1::Resources::LoadTexture("glass ao", "textures/pbr/plastic/ao.png"), 4);
        glassMat->Blend = true;
    ...
    {
        // post-processing
        Shader* defaultBlitShader = T1::Resources::LoadShader("blit", "shaders/screen_quad.vs", "shaders/default_blit.fs");
        defaultBlitMaterial = new Material(defaultBlitShader);
        
        // deferred
        deferredAmbientShader     = T1::Resources::LoadShader("deferred ambient", "shaders/deferred/screen_ambient.vs", "shaders/deferred/ambient.fs");
        deferredIrradianceShader  = T1::Resources::LoadShader("deferred irradiance", "shaders/deferred/ambient_irradience.vs", "shaders/deferred/ambient_irradience.fs");
        deferredDirectionalShader = T1::Resources::LoadShader("deferred directional", "shaders/deferred/screen_directional.vs", "shaders/deferred/directional.fs");
        deferredPointShader       = T1::Resources::LoadShader("deferred point", "shaders/deferred/point.vs", "shaders/deferred/point.fs");
        ...
        // shadows
        dirShadowShader = T1::Resources::LoadShader("shadow directional", "shaders/shadow_cast.vs", "shaders/shadow_cast.fs");
